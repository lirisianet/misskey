...
<SearchMarker path="/settings/privacy" :label="i18n.ts.privacy" :keywords="['privacy']" icon="ti ti-lock-open">
	<div class="_gaps_m">
		...
		<SearchMarker v-if="$i.policies.canAutoFollowBack" :keywords="['auto', 'follow', 'back']">
			<MkSwitch v-model="autoFollowBack" @update:modelValue="save()">
				...
				<template #caption><SearchKeyword>{{ i18n.ts.autoFollowBackDescription }}</SearchKeyword></template>
			</MkSwitch>
		</SearchMarker>

		<SearchMarker :keywords="['lockdown']">
			<FormSection>
				<template #label><SearchLabel>{{ i18n.ts.lockdown }}</SearchLabel><span class="_beta" style="vertical-align: middle;">{{ i18n.ts.beta }}</span></template>
