...
<div class="_gaps">
	...
	<FormSlot>
		...
		<div class="_gaps_s">
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.canUseRemoteAvatarDecorations, 'canUseRemoteAvatarDecorations'])">
				<template #label>{{ i18n.ts._role._options.canUseRemoteAvatarDecorations }}<span class="_beta">{{ i18n.ts._cherrypick.function }}</span></template>
				<template #suffix>
					<span v-if="role.policies.canUseRemoteAvatarDecorations.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.canUseRemoteAvatarDecorations.value ? i18n.ts.yes : i18n.ts.no }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.canUseRemoteAvatarDecorations)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.canUseRemoteAvatarDecorations.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkSwitch v-model="role.policies.canUseRemoteAvatarDecorations.value" :disabled="role.policies.canUseRemoteAvatarDecorations.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts.enable }}</template>
					</MkSwitch>
					<MkRange v-model="role.policies.canUseRemoteAvatarDecorations.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
						<template #label>{{ i18n.ts._role.priority }}</template>
					</MkRange>
				</div>
			</MkFolder>
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.canReadFollowHistory, 'canReadFollowHistory'])">
				...
				<template #suffix>
					...
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.canReadFollowHistory)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.canReadFollowHistory.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkSwitch v-model="role.policies.canReadFollowHistory.value" :disabled="role.policies.canReadFollowHistory.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts.enable }}</template>
					</MkSwitch>
