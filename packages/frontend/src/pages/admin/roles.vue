				...
				<MkFolder>
					...
					<div class="_gaps_s">
						...
						<MkFolder v-if="matchQuery([i18n.ts._role._options.canUseRemoteAvatarDecorations, 'canUseRemoteAvatarDecorations'])">
							...
							<template #suffix>{{ policies.canUseRemoteAvatarDecorations ? i18n.ts.yes : i18n.ts.no }}</template>
							<MkSwitch v-model="policies.canUseRemoteAvatarDecorations">
								<template #label>{{ i18n.ts.enable }}</template>
							</MkSwitch>
						</MkFolder>
						...
						<MkFolder v-if="matchQuery([i18n.ts._role._options.canRequestCustomEmojis, 'canRequestCustomEmojis'])">
							<template #label>{{ i18n.ts._role._options.canRequestCustomEmojis }} <span class="_beta">{{ i18n.ts._cherrypick.function }}</span></template>
